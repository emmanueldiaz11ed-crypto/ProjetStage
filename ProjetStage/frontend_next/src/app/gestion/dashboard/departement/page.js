...
export default function VueDepartement() {
  const [filters, setFilters] = useState({ departement: "", annee: "" });
  const [options, setOptions] = useState({ departements: [], annees: [] });
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <h1 className="text-2xl font-bold mb-4">Vue Département</h1>
      ...
      {data ? (
        <>
          ...
          <div className="grid gap-6 lg:grid-cols-2 mb-8">
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <h2 className="text-lg font-semibold mb-4">Scores par département</h2>
              <div className="overflow-x-auto">
                <table className="min-w-full text-left text-sm">
                  ...
                  <tbody>
                    {data.scores_depts?.slice(0, 8).map((row, index) => (
                      <tr key={index} className="border-t border-slate-200">
                        <td className="px-3 py-2">{row.departement || row.nom || "-"}</td>
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <h2 className="text-lg font-semibold mb-4">Tableau des UEs</h2>
              <div className="overflow-x-auto">
                <table className="min-w-full text-left text-sm">
                  <thead className="bg-slate-50 text-slate-600">
                    <tr>
                  ...
                  <tbody>
                    {data.tableau_ue?.slice(0, 8).map((row, index) => (
                      <tr key={index} className="border-t border-slate-200">
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="mb-3 text-sm font-semibold text-slate-600">Évolution des cohortes</div>
              <img
                src={getFigureUrl("courbe_cohortes", figureParams)}
                alt="Évolution cohortes"
                className="h-72 w-full rounded-xl object-contain border border-slate-200 bg-slate-50"
              />
            </div>
          </div>
        ...
        !loading && <div className="rounded-2xl border border-slate-200 bg-white p-6 text-slate-600 shadow-sm">Pas de données pour ce département.</div>
      )}
    </div>
  );
}
