...
export default function ImportUEExcel({onSuccess}) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (onSuccess && Array.isArray(data.ues)) {
        onSuccess(data.ues);
      }
    } catch (error) {
      const message = error?.response?.data?.detail || error?.message ||
        "❌ Erreur lors de l’importation. Revoyez le format du fichier.";
      setResult({ message });
      console.error(error);
  ...
  {result && (
    ...
    >
      ...
      {result.added !== undefined && (
        <p className="mt-0.5">
          <strong>{result.added}</strong> ligne(s) ajoutée(s) • <strong>{result.updated}</strong> ligne(s) mise(s) à jour
          {result.lignes_valides !== undefined && (
            <> • <strong>{result.lignes_valides}</strong> ligne(s) valides</>
          )}
        </p>
      )}
      {result.warnings && result.warnings.length > 0 && (
        <p className="mt-0.5 text-yellow-700 text-[10px]">
